...
export interface IRequestDocument extends IRequest, Document {
  ...
  cancel(cancelledBy: string, reason?: string): Promise<void>;
    ...
    items: [
      {
        item_id: {
          ...
          ref: "Item",
          validate: {
            validator: async function (this: any, itemId: string) {
...
RequestMongooseSchema.pre("save", async function (next) {
  ...
  console.log("[Request Model] Pre-save hook triggered");
  console.log("[Request Model] isNew:", doc.isNew);
  console.log("[Request Model] current request_id:", doc.request_id);

  // Only generate a proper request_id if it's a new document or has a temporary ID
  if (doc.isNew || !doc.request_id || doc.request_id.startsWith("REQ-TEMP-")) {
    try {
      console.log("[Request Model] Generating proper request_id");

      // Find the last request to get the next number
      const lastRequest = await mongoose
        .model("Request")
        .findOne({ request_id: { $regex: /^REQ-\d{5}$/ } })
        .sort({ request_id: -1 })
        .lean();

      console.log("[Request Model] Last request found:", lastRequest);

      let nextNumber = 1;
      if (lastRequest && (lastRequest as any).request_id) {
        const lastRequestId = (lastRequest as any).request_id;
        console.log("[Request Model] Last request_id:", lastRequestId);

        // Extract the number from REQ-XXXXX format
        const match = lastRequestId.match(/REQ-(\d+)/);
        if (match && match[1]) {
          nextNumber = parseInt(match[1], 10) + 1;
          console.log(
            `[Request Model] Last request number: ${match[1]}, next: ${nextNumber}`,
          );
        }
      }
      ...
      console.log(`[Request Model] Generated request_id: ${doc.request_id}`);
      next();
    } catch (error) {
      console.error("[Request Model] Error generating request_id:", error);
      // Fallback: use timestamp
      const timestamp = Date.now().toString().slice(-8);
      doc.request_id = `REQ-${timestamp}`;
      console.log(
        `[Request Model] Using fallback request_id: ${doc.request_id}`,
      );
      next();
    }
  } else {
    console.log(`[Request Model] Using existing request_id: ${doc.request_id}`);
    next();
  }
});
...
RequestMongooseSchema.pre("save", async function (next) {
  ...
  try {
    ...
    for (const item of doc.items) {
      ...
      item.requires_prescription = dbItem.requires_prescription || false;
    }

    // Update has_prescription flag
    doc.has_prescription = doc.items.some((item) => item.requires_prescription);

    // Update certificate flags based on item requirements
    const anyRequireMedCert = await checkIfAnyItemRequiresMedCert(doc.items);
    const anyRequireBrgyCert = await checkIfAnyItemRequiresBrgyCert(doc.items);

    doc.has_medical_cert = anyRequireMedCert;
    doc.has_barangay_cert = anyRequireBrgyCert;
...
// Helper function to check if any item requires medical certificate
async function checkIfAnyItemRequiresMedCert(
  items: IRequestItem[],
): Promise<boolean> {
  for (const item of items) {
    const dbItem = await ItemModel.findOne({ item_id: item.item_id });
    if (dbItem && dbItem.requires_med_cert) {
      return true;
    }
  }
  return false;
...
// Helper function to check if any item requires barangay certificate
async function checkIfAnyItemRequiresBrgyCert(
  items: IRequestItem[],
): Promise<boolean> {
  for (const item of items) {
    const dbItem = await ItemModel.findOne({ item_id: item.item_id });
    if (dbItem && dbItem.requires_brgy_cert) {
      return true;
    }
  }
  return false;
  ...
  if (
    ...
    try {
      ...
      const lastQueued = await mongoose
        .model("Request")
        .findOne({ status: "In Queue" }, { queue_number: 1 })
        .sort({ queue_number: -1 })
        .lean();

      let nextQueueNumber = 1;
      if (lastQueued && (lastQueued as any).queue_number) {
        nextQueueNumber = (lastQueued as any).queue_number + 1;
      }
...
// ============ MODEL ============
// Properly typed model export
const Request =
  (mongoose.models.Request as mongoose.Model<IRequestDocument>) ||
  mongoose.model<IRequestDocument>("Request", RequestMongooseSchema);

export const RequestModel = Request;
